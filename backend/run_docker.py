...
def graceful_shutdown(signum=None, frame=None):
    if docker_process and docker_process.poll() is None:
        ...
        try:
            ...
            subprocess.run(
                ["docker-compose", "-f", compose_file, "down"], check=False, timeout=30
            )
...
def run_docker():
    ...
    if check_cuda_gpu_available():
        print(
            "CUDA-compatible GPU detected! Running Docker container with GPU support..."
        )
        docker_compose_path = get_resource_path("docker-compose.gpu.yml")
        use_gpu = "true"
    else:
        print(
            "No CUDA-compatible GPU detected. Running Docker container in CPU-only mode..."
        )
    ...
    if not os.path.exists(docker_compose_path):
        show_error_message(
            "No docker-compose file found. Please ensure you're in the correct directory with the Docker Compose file."
        )
        return
    ...
    try:
        ...
        if (
            docker_process.returncode != 0 and docker_process.returncode != 130
        ):  # 130 is the return code for SIGINT
            show_error_message(
